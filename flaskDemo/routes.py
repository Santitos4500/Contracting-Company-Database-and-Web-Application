...
from flaskDemo.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, CreateProjectForm, UpdateSkillForm
...
#This home route is currently set up to show all projects in the database
@app.route("/home")
def home():
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT * FROM project")
    if resultValue > 0:
        projectResults = cur.fetchall()
        return render_template('home.html', projectResults=projectResults)

#Register route using flask login
# #Added userType to allow us to distinguish between contractor and customer using user_type
